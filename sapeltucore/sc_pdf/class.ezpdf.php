...
function Export2CSV($vaArray,$lShowHeader=true){
  ...
  foreach($vaArray as $key=>$value){
    ...
    foreach ($value as $cMeKey => $cMeValue) {
      if( substr($cMeValue, 0,1) == '0' && strlen($cMeValue) > 1  ){
        //$value[$cMeKey] = "'" . $cMeValue ; 
      }
    }
    $c = '"' . implode('","', $this->getText($value)) . '"' . chr(13) . chr(10) ;    
...
function _construct($paper='LETTER',$orientation='portrait',$vaOption=array(),$pagenumber='1',$nPaperIncWidth=0,$nPaperIncHeight=0){
  ...
  if (!is_array($paper)){
    ...
    if(!empty($nPaperIncWidth) && !empty($nPaperIncHeight)){
      $nPaperWidth = $nPaperIncWidth * 72;
      $nPaperHeight = $nPaperIncHeight * 72;
      $size = array(0,0,$nPaperWidth,$nPaperHeight) ;
    }
  ...
  // Default Setting 
  $this->selectFont('./sapeltucore/sc_pdf/fonts/Helvetica.afm');
...
function ezSetMargins($top,$bottom,$left,$right){
  ...
  if ($this->y > $this->ez['pageHeight']-$top) $this->y = $this->ez['pageHeight']-$top;
  if ( $this->y < $bottom) $this->ezNewPage();
}  
...
function ezStartPageNumbers($x,$y,$size,$pos='left',$pattern='{PAGENUM} of {TOTALPAGENUM}',$num='',$vaArrayImage=array()){
  ...
  $vaArray  = array('x'=>$x,'y'=>$y,'pos'=>$pos,'pattern'=>$pattern,'num'=>$num,'size'=>$size) ;
  if(!empty($vaArrayImage)){
    $vaArray['lImage']    = $vaArrayImage['lImage'] ; 
    $vaArray['nWidth']    = $vaArrayImage['nWidth'] ;
    $vaArray['nHeight']   = $vaArrayImage['nHeight'] ; 
  } 
  $this->ez['pageNumbering'][$i][$this->ezPageCount]= $vaArray ;
  return $i;
}
...
function ezWhatPageNumber($pageNum,$i=0){
  ...
  if (!isset($this->ez['pageNumbering']))
  {
    $this->addMessage('WARNING: page numbering called for and wasnt started with ezStartPageNumbers');
    return 0;
  }
      ...
      foreach ($this->ezPages as $pageNum=>$id){
        ...
        if ($status){
          ...
          if(!isset($info['lImage'])){
            ...
            //$this->ezImage($info['pattern'],true,0,0,'height','5','left') ; 
            $this->addJpegFromFile($info['pattern'],$info['x'],$info['y'],$info['nWidth'],$info['nHeight']);
          }   
          $this->closeObject();
        }
        if ($status==2){
...
/**************************************************************************************************************************************************************************************************/
function ezPrvtTableColumnHeadings($cols1,$pos1,$maxWidth1,$height,$decender,$gap,$size,&$y,$optionsAll=array(),$vaHeader3){
  ...
  $nShowLines = $optionsAll ['showLines'] ;
  $lSecondHeader = false ;

  if(!empty($vaHeader3)){
    $nRow = 0 ;
    foreach($vaHeader3 as $key=>$value){
      $_va = explode(";",$value,2) ;
      $vaHeader ["KEY" . $nRow] = $_va [0] ;
      $vaHeader2 ["KEY" . $nRow] = "" ;
      if(isset($_va [1])){
        $vaHeader2 ["KEY" . $nRow] = $_va [1] ;
        $lSecondHeader = true ;
      }
      $nRow ++ ;
    } 
  ...
  $x = 0 ;
  foreach($cols1 as $key=>$value){
    $cols ["KEY" . $x] = $value ;
    $pos ["KEY" . $x] = $pos1 [$key] ;
    $maxWidth ["KEY" . $x] = $maxWidth1 [$key] ;
    $x ++ ;
  }
  $mx=0;
  ...
  $nCount = count($pos) - 1 ;
  if($nShowLines > 0) $this->line($pos["KEY0"]-$gap,$y,$pos ["KEY" . $nCount]+$maxWidth["KEY" . $nCount]+$gap,$y) ;
  ...
  while ($ok==0){
    ...
    foreach($cols as $colName=>$colHeading){
      if(++$nShow >= 1){
        $nColWidth = $maxWidth[$colName] ;
        $nShow = 0 ;
        if(isset($vaHeader [$colName])){
          $colHeading = $vaHeader [$colName] ;
          for($x=substr($colName,3)+1;$x<count($vaHeader);$x++){
            if($colHeading == $vaHeader ["KEY" . $x]){
              $nShow -- ;
              $nColWidth += $maxWidth["KEY" . $x] ;
            }else{
              $x = count($vaHeader) ;
            }
          }
        ...
        $this->ezText('<b>' . $colHeading . '</b>',$size,array('aleft'=> $pos[$colName],'aright'=>($nColWidth+$pos[$colName]),'justification'=>$justification));
        $dy = $y-$this->y;
        if ($dy>$mx) $mx=$dy;

        $vaVLine [$colName] = array("left"=>$pos [$colName]-$gap,"top"=>$y+$gap) ;
      }
    }
    $vaVLine ["xxLast"] = array("left"=>$pos [$colName]+$maxWidth[$colName]+$gap,"top"=>$y+$gap) ;
    $y = $y - $mx - $gap + $decender;

    $nHead2 = 0 ;
    if($lSecondHeader){
      $mx2 = 0 ;
      foreach($vaHeader2 as $key=>$value){
        if(!empty($value)){
          $vaLine2 [$key] = array("left"=>$pos [$key] + $maxWidth [$key],"top"=>$y) ;
          if($nShowLines > 0) $this->line($pos[$key]-$gap,$y,$pos[$key]+$maxWidth[$key]+$gap,$y) ;
          $this->ezSetY($y-$gap);
          $this->ezText('<b>' . $value . '</b>',$size,array('aleft'=> $pos[$key],'aright'=>(+$maxWidth[$key]+$pos[$key]),'justification'=>$justification));
          $dy = $y-$this->y;
          if ($dy>$mx2) $mx2=$dy;
        }
      }
      $y = $y - $mx2 - $gap + $decender;
      $nHead2 = $mx2 + $gap - $decender ;
      
      foreach($vaLine2 as $key=>$value){
        if($nShowLines > 0) $this->line($value ['left']+$gap,$value ['top'],$value ['left']+$gap,$value ['top']-$mx2-$gap-$gap+$decender) ;
      }
    }

    foreach($vaVLine as $key=>$value){
      if($nShowLines > 0) $this->line($value ['left'],$value ['top'],$value ['left'],$value ['top']-$mx-$gap-$gap+$decender+$decender-$nHead2) ;
    }
...
/**************************************************************************************************************************************************************************************************/
function ezImage($image,$lTop=false,$nHeight=0,$width = 0,$resize = 'full',$pad = 5,$just = 'center',
                $border = '',$vaMargin=array()){
  $vaMargin = array_merge(array("Top"=>0,"Left"=>0),$vaMargin) ; 
  $temp   = false ; 
  if (stristr($image,'://')){ 
    $cont = file_get_contents($image);        
    $image = tempnam(sys_get_temp_dir(), md5("php-pdf" . time()) );
    $fp2 = @ fopen($image, "w");
    fwrite($fp2, $cont);
    fclose($fp2);
    $temp = true;
  ...
  if ($type == "jpeg"){
    $this->addJpegFromFile($image,$this->ez['leftMargin'] + $padLeft + $offset + $vaMargin['Left'], 
                          $this->y + $this->getFontHeight($this->ez['fontSize']) - $padTop - $height + $vaMargin['Top'],$width);
  }else if ($type == "png"){
    $this->addPngFromFile($image,$this->ez['leftMargin'] + $padLeft + $offset + $vaMargin['Left'], 
                          $this->y + $this->getFontHeight($this->ez['fontSize']) - $padTop - $height + $vaMargin['Top'],$width);
  }
  if ($border != ''){
    ...
    $this->rectangle($this->ez['leftMargin'] + $padLeft + $offset + $vaMargin['Left'], 
                    $this->y + $this->getFontHeight($this->ez['fontSize']) - $padTop - $height + $vaMargin['Top'],$width,$height);
  }

  if($lTop) $this->nScTop = $height ; 
  $this->y = $this->y - $pad - $height+ $vaMargin['Top'];

  if ($temp) unlink($image);
}

function loadTemplate($templateFile){
  if (!file_exists($templateFile)) return -1;
...
function uline($info){
  ...
  $lineFactor=0.125; // the thickness of the line as a proportion of the height. also the drop of the line.
  switch($info['status']){
    case 'start':
    case 'sol':
...
function ezTable(&$data,$cols='',$title='',$options=''){
  // add a table of information to the pdf document $data is a two dimensional array
  ...
  // 'widthType'=>0,1, default is 0 (0 = percent, 1 = mm)
  ...
  // 'rowHeadGap' => 2 , the space on the top and bottom of Table Header
  ...
  // 'options' => array(<colname>=>array('caption'=>'colcaption','justification'=>'left','width'=>100,'link'=>linkDataName,'wrap'=>false),<colname>=>....)
  ...
  $defaults = array('shaded'=>0,'showLines'=>2,'shadeCol'=>array(0.8,0.8,0.8),'shadeCol2'=>array(0.7,0.7,0.7),'fontSize'=>10,'titleFontSize'=>11
    ,'titleGap'=>5,'lineCol'=>array(0,0,0),'gap'=>5,'xPos'=>'centre','xOrientation'=>'centre'
    ,'showHeadings'=>1,'widthType'=>0,'textCol'=>array(0,0,0),'width'=>0,'maxWidth'=>0,'cols'=>array(),'minRowSpace'=>-100,'rowGap'=>2,'rowHeadGap'=>2,'colGap'=>2
    ,'innerLineThickness'=>1,'outerLineThickness'=>1,'splitRows'=>0,'protectRows'=>1
    );

  foreach($defaults as $key=>$value){
    if (is_array($value)){
      if (!isset($options[$key]) || !is_array($options[$key])) $options[$key]=$value;
    } else {
  ...
  foreach($cols as $colName=>$colTitle){
    ...
    if ($w > $maxWidth[$colName]) $maxWidth[$colName]=$w;
  }
  ...
  foreach($maxWidth as $colName => $w){
    $vaHeader [$colName] = $colName ;
    if (isset($options['cols'][$colName])){
      if (isset($options['cols'][$colName]['caption'])) $vaHeader [$colName] = $options['cols'][$colName]['caption'] ;

      if (isset($options['cols'][$colName]['width'])){
        if ($options['cols'][$colName]['width'] > 0){
          $nTotalColWidth += $options['cols'][$colName]['width'] ;
        }else{
          $nColNotSet += 1 ;
        }
      }else{
    ...
    if(!isset($options['cols'][$colName]['wrap'])){
      $options['cols'][$colName]['wrap'] = false ;
    }
  }
  
  // Setting Jika Kolom Kosong
  if($nColNotSet > 0){
    $nTotalColWidth = max(100-$nTotalColWidth,0) ;
    $nWidthPerCol = round($nTotalColWidth / $nColNotSet  ,2) ;
    foreach($maxWidth as $colName => $w){
      if (isset($options['cols'][$colName])){
        if (isset($options['cols'][$colName]['width'])){
          if ($options['cols'][$colName]['width'] == 0){
            $options['cols'][$colName]['width'] = $nWidthPerCol ;
          }
        }
      }
    }
  ...
  foreach($maxWidth as $colName => $w){
    ...
    if (isset($options['cols'][$colName]) && isset($options['cols'][$colName]['width']) && $options['cols'][$colName]['width']>0){
      // Setting Colom Width Ke Persen
      if($options ['widthType'] == 0){
        $nColWidth = $this->ez['pageWidth'] - $this->ez['leftMargin'] - $this->ez['rightMargin'] ;
        $nColWidth = round($nColWidth * $options['cols'][$colName]['width'] / 100,2) ;      
        $options['cols'][$colName]['width'] = $nColWidth ;
      }else{
        $options['cols'][$colName]['width'] = round((($options['cols'][$colName]['width'] / 25.4 ) * 72) * 1.125,2) ;
      }
  ...
  if ($options['showHeadings']){
    ...
    $headingHeight = $this->ezPrvtTableColumnHeadings($cols,$pos,$maxWidth,$height,$decender,$options['rowHeadGap'],$options['fontSize'],$y,$options,$vaHeader);
    ...
    while(!$abortTable && ($newPage || $newRow)){      
      ...
      if ($newPage || $y<$this->ez['bottomMargin'] || (isset($options['minRowSpace']) && $y<($this->ez['bottomMargin']+$options['minRowSpace'])) ){
        ...
        if ($options['showLines']){
          if (!$options['showHeadings'] || $options ['showLines'] < 2){
            $y0=$y1;
          }else{
            $y0 = $y1 - 2 ;
          }
        ...
        $mx=0;
        if ($options['showHeadings']){
          $this->ezPrvtTableColumnHeadings($cols,$pos,$maxWidth,$height,$decender,$options['rowHeadGap'],$options['fontSize'],$y,$options,$vaHeader);
          $y1=$y;
        } else {
          $y1=$y0;
        }
        ...
        foreach ($lines as $line){
          ...
          while (strlen($line) || $start){
            ...
            if ($colNewPage){
              ...
              $line=$this->addTextWrap($pos[$colName],$this->y,$maxWidth[$colName],$options['fontSize'],$line,$just,0,0,$options['cols'][$colName]['wrap']);
            }
          }
        }
...
function ezHeader($cText,$vaOption = ''){
  ...
  foreach($vaDefault as $key=>$value){
    if (is_array($value)){
      if (!isset($vaOption[$key]) || !is_array($vaOption[$key])){
      $vaOption[$key]=$value;
      }
    } else {
      if (!isset($vaOption[$key])){
      $vaOption[$key]=$value;
      }
    }
  }

  $nTextWidth = ($this->getTextWidth($vaOption['fontSize'],$cText)) ;
  $nTextHeight = ($this->getFontHeight($vaOption['fontSize'])) ;
  
  if ($vaOption['justification'] == 'center'){
    $nMiddle = ($this->ez['pageWidth']-$this->ez['rightMargin'])/2+($this->ez['leftMargin'])/2;    
    $nTextWidth = $nTextWidth / 2 ;
    $nLeft = $nMiddle + $nTextWidth ;
  }else if($vaOption['justification'] == 'left'){
    $nLeft = ($this->ez['leftMargin']) + $nTextWidth ;
  }else{
    $nLeft = ($this->ez['pageWidth']-$this->ez['rightMargin']) ;
  }  
  
  $nCount = 5 ;
  $nTop = ($this->ez['pageHeight'] - $this->ez['topMargin']) - $nTextHeight - ($this->nScTop) ;  
  if ($this->nPageHeaderLine !== 0 ){
    $nTop += 5 ;
    $nCount = 0 ;
  }
  $this->nPageHeaderLine += 1 ;  
  $this->ezStartPageNumbers($nLeft,$nTop,$vaOption['fontSize'],'left',$cText,1) ;
  
  if($vaOption['newLine']){
    $nTop = $this->ez['topMargin'] + $nTextHeight + $nCount ; 
  }else{
    $nTop = $this->ez['topMargin'] + $nCount ;
  }
  $nBottom = $this->ez['bottomMargin'] ;
  $nLeft = $this->ez['leftMargin'] ;
  $nRight = $this->ez['rightMargin'] ;
  $this->ezSetMargins($nTop,$nBottom,$nLeft,$nRight) ;   
}
...
function ezLogoHeaderPage($cLoc,$nLeft=0,$nTop=0,$nWidth=0,$nHeight=0){
  if($cLoc !== ""){
    $nLeft  = $this->ez['leftMargin'] + $nLeft ;
    $nTop   = $this->ez['pageHeight'] - $nTop ;  

    if (stristr($cLoc,'://')){ 
      $cont = file_get_contents($cLoc);        
      $cLoc = tempnam(sys_get_temp_dir(), md5("php-pdf" . time()) );
      //$cLoc = sys_get_temp_dir() . md5("php-pdf".time) . ".jpg" ; 
      $fp2 = @ fopen($cLoc, "w");
      fwrite($fp2, $cont);
      fclose($fp2); 
    }
...
function ezHeaderRepeat($cText="",$nLoop=0){
  for($i = 0 ; $i < $nLoop ; $i++){
    $this->ezHeader($cText) ; 
  }
}

function ezSetTextNumber($cText,$nPlusTop=0,$cPos='right',$nFont=8){
  $nWidth     = $this->getTextWidth($nFont,$cText) ;
  if($cPos == 'right'){
    $nLeft      = $this->ez['pageWidth'] - $this->ez['rightMargin'] ;
  }else{
  ...
  $nTop       = $this->ez['bottomMargin'] + $nPlusTop; 
